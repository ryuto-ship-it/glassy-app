...
//    have specific, curated copy for (건조/피곤/다크서클/불면/소화).
// 2) A general body-part/pain parser (parseSymptomAndRecommend) that covers
//    everything else — "허리아파", "무릎이 아파요", "목 통증" etc. — by
//    regex-extracting the symptom word and mapping it to a product
//    category, with a safe generic fallback for unmapped body parts and a
//    guidance fallback for completely unrelated input (greetings, chatter).
//    This guarantees the chat never has a "no reply" dead end.
//
...
// Trigger phrases in roughly longest-first order so the alternation
// doesn't stop early on a shorter prefix (e.g. "아파요" before "아파").
const PAIN_TRIGGERS = ['아파요', '아파', '아픔', '아픈', '통증'];

const PAIN_PATTERN = new RegExp(`([가-힣]{1,6}?)(?:이|가)?\\s*(?:${PAIN_TRIGGERS.join('|')})`);
...
//   1. Known keyword (건조/피곤/다크서클/불면/소화 등) → curated reply.
//   2. "OO 아파요/아픔/통증" pattern → mapped category, or a safe generic
//      recommendation if the body part isn't one we have a mapping for.
//   3. Nothing matched at all (greeting, chatter, random text) → guidance
//      reply with no product attached.
export function parseSymptomAndRecommend(input: string): ChatResponse {
  ...
  return { reply: CHAT_FALLBACK_REPLY, productIds: [] };
}
